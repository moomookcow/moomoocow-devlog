...
디자인 시스템 상세 명세: [DESIGN_SYSTEM.md](./DESIGN_SYSTEM.md)
...
- 폰트 정책:
  - 주요 한글 타이틀/헤딩은 Song Myung 적극 적용
  - 본문/폼/메타 텍스트는 Noto Sans KR 유지
- UI 컴포넌트 정책:
  - input/button/textarea/select/dialog 등 인터랙션 요소는 shadcn/ui 기반 컴포넌트만 사용
  - raw HTML 컨트롤은 직접 사용하지 않고 공용 UI 컴포넌트로 래핑해서 사용
  - 다이얼로그 포함 대부분 컴포넌트는 shadcn/ui를 사용하되, 최종 스타일은 반드시 우리 디자인 시스템 토큰/타이포/간격 규칙에 맞춰 커스터마이즈
...
- 댓글 작성 UI/기능 제공
- 상세 페이지 로딩 시 제목/본문/TOC/이전·다음 글 영역 스켈레톤 제공
...
- 다크모드 토글 기본 구조
- 폰트 롤아웃 1차 적용(공개/어드민 핵심 타이틀 Song Myung 반영)
- shadcn/ui 컴포넌트 사용 기준 확정 및 공용 규칙 고정

완료 기준:
- 로그인/로그아웃/비인가 차단이 3회 연속 안정 동작
- 모바일/데스크톱 공통 레이아웃 깨짐 없음
- 핵심 화면에서 폰트 정책(Song Myung display + sans body) 일관 적용
- 신규 인터랙션 UI가 shadcn/ui 정책을 위반하지 않음
